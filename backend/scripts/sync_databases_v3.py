#!/usr/bin/env python3
"""
Notion Database Sync Script V3 - Updated for simplified schema
Syncs Notion databases to the new schema without workspace concept.

Usage:
    python sync_databases_v3.py --config config/databases.toml
    python sync_databases_v3.py --database-id <notion_database_id>
    python sync_databases_v3.py --dry-run
...
from database_v3 import get_db, init_db
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('../logs/sync_v3.log')
    ]
)
logger = logging.getLogger(__name__)


class DatabaseSyncConfigV3:
    """Configuration for a single database sync in V3 schema."""
...
class NotionDatabaseSyncerV3:
    """Main syncer class for V3 schema."""
    ...
    async def initialize(self):
        ...
        logger.info("Initializing services for V3 schema...")
    ...
    async def sync_database(self, config: DatabaseSyncConfigV3) -> Dict[str, Any]:
    ...
    async def _register_notion_database(self, config: DatabaseSyncConfigV3):
    ...
    async def _process_page(self, page: Dict[str, Any], config: DatabaseSyncConfigV3) -> Dict[str, str]:
    ...
    async def _save_page_to_database(self, page: Dict[str, Any], content: str, config: DatabaseSyncConfigV3):
    ...
    async def _process_document_chunks(self, document_id: str, title: str, content: str, config: DatabaseSyncConfigV3):
    ...
    async def run_sync(self, configs: List[DatabaseSyncConfigV3]) -> Dict[str, Any]:
...
def load_config(config_path: str) -> List[DatabaseSyncConfigV3]:
    ...
    try:
        ...
        for db_config in config_data.get('databases', []):
            config = DatabaseSyncConfigV3(db_config)
            if config.is_valid():
...
def create_single_database_config(database_id: str) -> DatabaseSyncConfigV3:
        ...
        try:
            ...
            for db_config in config_data.get('databases', []):
                if db_config.get('database_id') == database_id:
                    ...
                    return DatabaseSyncConfigV3(db_config)
        except Exception as e:
    ...
    return DatabaseSyncConfigV3(config_dict)


async def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Sync Notion databases to V3 schema')
    ...
    # Run sync
    syncer = NotionDatabaseSyncerV3(dry_run=args.dry_run)
